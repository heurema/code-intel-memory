...
# code-intel-memory setup script (macOS + Linux)
...
BINARY_NAME="code-intel-memory"
SOURCE_DIR="$HOME/.local/share/code-intel-memory"
...
download_binary() {
    ...
    local asset="code-intel-memory-${platform}.tar.gz"
...
build_from_source() {
    ...
    (cd "$SOURCE_DIR" && scripts/build.sh && cp build/c/code-intel-memory "${INSTALL_DIR}/${BINARY_NAME}")
...
configure_claude() {
    ...
    printf "%s" "${BOLD}Configure Claude Code to use code-intel-memory? [y/N] ${RESET}"
    read -r answer
    if [[ ! "$answer" =~ ^[Yy]$ ]]; then
        echo ""
...
JSONEOF
    ...
    if command -v jq &>/dev/null; then
        ...
        if [ -f "$settings_file" ]; then
            local tmp
            tmp=$(mktemp)
            jq --argjson entry "$mcp_entry" '.mcpServers["code-intel-memory"] = $entry' "$settings_file" > "$tmp"
            mv "$tmp" "$settings_file"
        else
            echo "{}" | jq --argjson entry "$mcp_entry" '.mcpServers["code-intel-memory"] = $entry' > "$settings_file"
        fi
...
if os.path.exists(path):
    with open(path) as f:
        data = json.load(f)
data.setdefault('mcpServers', {})['code-intel-memory'] = json.loads('$mcp_entry')
...
echo ""
echo "${BOLD}code-intel-memory installer${RESET}"
echo ""
...
info "  rm -rf ~/.cache/code-intel-memory/  # graph database"
