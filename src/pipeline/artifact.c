...
/* Build path: <repo>/<dir>/<name> into caller-owned buf. */
static bool artifact_path_in_dir(char *buf, size_t bufsz, const char *repo_path, const char *dir,
                                 const char *name) {
    int n = snprintf(buf, bufsz, "%s/%s/%s", repo_path, dir, name);
    return n >= 0 && (size_t)n < bufsz;
}

/* Build path: <repo>/.code-intel-memory/<name> into caller-owned buf. */
static bool artifact_path(char *buf, size_t bufsz, const char *repo_path, const char *name) {
    return artifact_path_in_dir(buf, bufsz, repo_path, CBM_ARTIFACT_DIR, name);
}

static bool artifact_legacy_path(char *buf, size_t bufsz, const char *repo_path,
                                 const char *name) {
    return artifact_path_in_dir(buf, bufsz, repo_path, CBM_LEGACY_ARTIFACT_DIR, name);
}

static bool artifact_existing_path(char *buf, size_t bufsz, const char *repo_path,
                                   const char *name) {
    if (!artifact_path(buf, bufsz, repo_path, name)) {
        return false;
    }
    ...
    char legacy[CBM_SZ_4K];
    if (!artifact_legacy_path(legacy, sizeof(legacy), repo_path, name)) {
        return false;
    }
    if (!cbm_file_exists(legacy)) {
        return true;
    }

    int n = snprintf(buf, bufsz, "%s", legacy);
    return n >= 0 && (size_t)n < bufsz;
}
...
static int read_metadata_version(const char *repo_path) {
    ...
    artifact_existing_path(meta_path, sizeof(meta_path), repo_path, CBM_ARTIFACT_META);
...
static void ensure_gitattributes(const char *repo_path) {
    ...
    if (fd < 0) {
        ...
        FILE *fp = fdopen(fd, "w");
        if (fp) {
            (void)fputs("# Auto-generated by code-intel-memory\n"
...
int cbm_artifact_export(const char *db_path, const char *repo_path, const char *project_name,
    ...
    /* Ensure .code-intel-memory/ directory exists */
...
int cbm_artifact_import(const char *repo_path, const char *cache_db_path) {
    ...
    char zst_path[CBM_SZ_4K];
    artifact_existing_path(zst_path, sizeof(zst_path), repo_path, CBM_ARTIFACT_FILENAME);
